...
这是一个生成类产品：**同一个想法，每次生成的作品都不一样**。模板引擎只是剧本的执行器。

| 层 | 内容 | 实现 |
|---|---|---|
| 理解生成 | 把孩子的话变成结构化需求 | LLM 语义归一（可配置）→ 规则引擎兜底 |
| **内容生成** | **作品基因 Genome**：开场白、通关台词、关卡名、游戏中途的剧情事件（星星雨/顺风/加油）、物件命名、故事书四页正文、介绍页趣味问答、邀请函文案 | 确认后与构建动画并行生成：LLM 创作（`temperature 0.9`）→ 校验修补 → 程序化生成器兜底（离线也每次不同） |
| 视觉生成 | 终稿魔法图、作品封面 | 图片生成 API → 手绘图兜底 |

Genome 存入作品档案与分享链接（生成产物的一部分），游戏运行时按剧本"上演"：
事件到点弹横幅、星星雨撒粒子、顺风加速、通关时说出这个作品独有的台词。
