...
// ─────────────────────────────────────────────────────────────
function MacToolbar({ title = 'Folder', noSidebar = false }) {
  return (
    <div style={{
      display: 'flex', gap: 8, alignItems: 'center', padding: 8, flexShrink: 0,
    }}>
      {/* leave room for overlaid traffic lights when there is no sidebar */}
      {noSidebar && <div style={{ width: 66, flexShrink: 0 }} />}
      {/* title */}
      <div style={{
...
function MacWindow({
  ...
  return (
    <div style={{
      ...
      {sidebar && <MacSidebar>{sidebar}</MacSidebar>}
      {!sidebar && <MacTrafficLights style={{ position: 'absolute', top: 14, left: 16, zIndex: 5 }} />}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <MacToolbar title={title} noSidebar={!sidebar} />
